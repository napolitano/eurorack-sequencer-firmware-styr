...
```bash
g++ -std=c++17 -O2 -Wall -Wextra -pedantic src/tools/fonts/glyphtext_to_bin.cpp -o build/tools/fonts/glyphtext_to_bin
```

Convert an existing 8x8 file using defaults:

```bash
build/tools/fonts/glyphtext_to_bin assets/fonts/source/8x8font.txt /tmp/font8x8.bin
```

Convert a 5x5 file via CLI size:

```bash
build/tools/fonts/glyphtext_to_bin --size 5x5 assets/fonts/source/5x5symbols.txt /tmp/font5x5.bin
```
