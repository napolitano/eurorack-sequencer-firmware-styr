...
That is the complete maintainer trigger. Do not start the Analyst, Manual SME, UX/IA Reviewer, Technical Reviewer, or Editor manually.
...
GitHub issue assignment creates the one Draft PR for the task. The Orchestrator first regenerates current screenshots when the mode requires it, then uses the custom-agent `agent` tool to run the internal specialists in order:
...
Styr Documentation Orchestrator
        ...
        |
        +--> complete screenshot regeneration
        |
...
### Manual UX and Information Architecture Reviewer

The UX/IA Reviewer asks whether the result is actually a good end-user manual. It protects useful existing prose, normal-workflow-first explanations, coherent chapter roles, meaningful examples, and screenshots that teach rather than merely exist. It may improve deterministic capture states but cannot change product behavior.

After this stage the Orchestrator runs `python3 toolchain/regenerate_manual_screenshots.py` to regenerate the complete screenshot corpus and validate the resulting assets before factual Technical Review.

### Technical Reviewer

The Technical Reviewer verifies and, when evidence is clear, corrects user-facing facts against implementation, tests, UI state, defaults/ranges, and the freshly regenerated screenshots/capture definitions. A factual blocker stops the pipeline before editorial review.
...
The five specialists return structured results to the Orchestrator. Internal impact/review handoffs are kept in agent task context and are not committed to the repository.
...
The Orchestrator never advances past an incomplete or blocked specialist result. It may reinvoke an incomplete Analyst or SME automatically. A blocked UX/IA or Technical Review requires human intervention.
