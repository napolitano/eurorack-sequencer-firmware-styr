/**
 * @file WindowPainter.cpp
 * @brief Implements the WindowPainter component used by the sequencer UI painter.
 *
...
namespace {
    ...
    static char updateClockAnimation(const Engine &engine, const bool isRunning) {
        ...
        // Ensure both arguments to std::max are the same type to avoid
        // template deduction issues on some toolchains (unsigned int vs uint32_t).
        const uint32_t stepDivisor = std::max<uint32_t>(uint32_t(1), engine.noteDivisor());
...
static void drawInvertedText(Canvas &canvas, int x, int y, const char *text, bool inverted = true) {
    ...
    if (inverted) {
        canvas.fillRect(x - 1, y - 6, canvas.textWidth(text) + 1, 7);
...
void WindowPainter::drawFunctionKeys(Canvas &canvas, const char *names[], const KeyState &keyState, int highlight) {
    ...
    const int footerTextY = PageHeight - 2;
	...
	for (int i = 1; i < FunctionKeyCount; i++) {
	    canvas.vline(i * 51, PageHeight - FooterHeight, FooterHeight);
	}
    ...
    for (int i = 0; i < FunctionKeyCount; ++i) {
        ...
        // Support a simple disabled-label marker: labels starting with '~'
        // will be drawn in a dim color and are never interactive.
        bool disabled = (label[0] == '~');
        const char *text = disabled ? (label + 1) : label;

        bool isPressed = false;
        if (!disabled) {
            isPressed = keyState[Key::F0 + i];
            if (highlight >= 0) {
        ...
        // Use dim color for disabled labels
        if (disabled) canvas.setColor(UI_COLOR_DIM_MORE);
        else canvas.setColor(UI_COLOR_ACTIVE);

        const int textX = left + (width - canvas.textWidth(text)) / 2;
        canvas.drawText(textX, footerTextY, text);
    }
}
...
void WindowPainter::drawClock(Canvas &canvas, const Engine &engine) {
    ...
    canvas.drawText(2, 8 - 1, FixedStringBuilder<24>(TXT_INFO_TEMPO, name, engine.tempo()));
    canvas.drawText(20, 8 - 1, animationText);
}
