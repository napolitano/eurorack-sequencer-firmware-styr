...
/**
 * @brief Implements the note sequence edit page user-interface page.
 */
class NoteSequenceEditPage : public BasePage {
public:
    /**
     * @brief Constructs a NoteSequenceEditPage instance.
     *
...
private:
    ...
    /**
     * @brief Returns the all selected steps active.
     *
     * @return `true` if all selected steps active; otherwise `false`.
     */
    bool allSelectedStepsActive() const;
    /**
     * @brief Sets the selected steps gate.
     *
    ...
    NoteSequenceListModel _listModel; ///< List model backing the list model controls shown on this page.

    StepSelection<CONFIG_STEP_COUNT> _stepSelection; ///< Step-selection mask/range manipulated by this sequence-edit page.

    Container<NoteSequenceBuilder> _builderContainer; ///< In-place variant storage that owns the active builder object without heap allocation.
};
