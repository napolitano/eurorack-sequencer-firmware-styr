/**
 * @file Engine.cpp
 * @brief Implements the Engine component used by the sequencer engine.
 *
...
{
    //printf("Engine::Engine: _model at %p, _project at %p\n", (void*)&_model, (void*)&_project); fflush(stdout);
    ...
    for (int trackIndex = 0; trackIndex < CONFIG_TRACK_COUNT; ++trackIndex) {
        ...
        if (!_trackEngines[trackIndex] || _trackEngines[trackIndex]->trackMode() != track.trackMode()) {
            ...
            case Track::TrackMode::Lfo:
                trackEngine = trackContainer.create<LfoTrackEngine>(*this, _model, track, linkedTrackEngine);
                break;
