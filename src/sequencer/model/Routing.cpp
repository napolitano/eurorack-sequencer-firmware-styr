/**
 * @file Routing.cpp
 * @brief Implements the Routing component used by the sequencer model.
 *
...
Routing::Routing(Project &project) :
    _project(project)
{
    //printf("Routing::Routing: _project at %p\n", (void*)&_project); fflush(stdout);
}
        ...
        for (int trackIndex = 0; trackIndex < CONFIG_TRACK_COUNT; ++trackIndex) {
            if (tracks & (1<<trackIndex)) {
                ...
                case Track::TrackMode::Lfo:
                    // No routed write for LFO tracks
                    break;
...
static std::array<uint8_t, size_t(Routing::Target::Last)>& routedSet() {
    static std::array<uint8_t, size_t(Routing::Target::Last)> s{};
    return s;
}
static const TargetInfo* targetInfos() {
    static const TargetInfo t[int(Routing::Target::Last)] = {
        ...
        // LFO targets
        [int(Routing::Target::LfoHi)]        = { -500, 500, 1000, 1000, 10 },
        [int(Routing::Target::LfoLow)]       = { -500, 500, -1000, -1000, 10 },
        [int(Routing::Target::LfoSpeed)]     = { 1, 128, 16, 16, 1 },
        [int(Routing::Target::LfoShape)]     = { 0, int(LfoTrack::Waveform::Last)-1, 0, 0, 1 },
        [int(Routing::Target::LfoMode)]      = { 0, int(LfoTrack::Mode::Last)-1, 0, 0, 1 },
        [int(Routing::Target::LfoClip)]      = { 0, 1, 0, 0, 1 },
        [int(Routing::Target::LfoPulseWidth)] = { 1, 99, 50, 50, 1 },
        // Sequence targets
