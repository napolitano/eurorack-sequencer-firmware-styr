/**
 * @file Renderer.cpp
 * @brief Implements the Renderer component used by the simulator frontend.
 *
    ...
    _font(-1)
{
    ...
    // GLEW can leave GL_INVALID_ENUM set when initializing a core profile.
    glGetError();
#endif
...
void Renderer::drawArc(const Vector2f &pos, const Vector2f &size, float rotation) {
    nvgBeginPath(_nvg);
    nvgArc(_nvg, pos.x() + size.x() * 0.5f, pos.y() + size.y() * 0.5f, size.x() * 0.5f, -0.5f * PI, (rotation - 0.5f) * PI, rotation > 0.f ? NVG_CW : NVG_CCW);
    nvgStroke(_nvg);
}
