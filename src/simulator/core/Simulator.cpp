/**
 * @file Simulator.cpp
 * @brief Implements the Simulator component used by the simulator core.
 *
...
void Simulator::screenshot(const std::string &filename, int scale) {
    screenshotRegion(filename, 0, 0, CONFIG_LCD_WIDTH, CONFIG_LCD_HEIGHT, scale);
}

void Simulator::screenshotRegion(const std::string &filename, int x, int y, int width, int height, int scale) {
    scale = std::max(1, scale);

    x = std::max(0, std::min(x, CONFIG_LCD_WIDTH));
    y = std::max(0, std::min(y, CONFIG_LCD_HEIGHT));
    width = std::max(0, std::min(width, CONFIG_LCD_WIDTH - x));
    height = std::max(0, std::min(height, CONFIG_LCD_HEIGHT - y));
    if (width == 0 || height == 0) {
        return;
    }

    const int outputWidth = width * scale;
    const int outputHeight = height * scale;
    std::unique_ptr<uint8_t[]> pixelBuffer(new uint8_t[outputWidth * outputHeight]);
    const uint8_t *src = targetState().lcd.state.data();

    // Documentation screenshots must remain pixel-exact. The LCD stores
    // 4-bit intensities (0..15); multiplying by 17 maps them exactly to
    // 8-bit grayscale. Scaling is nearest-neighbour by explicit pixel
    // replication, so no anti-aliasing or interpolation is introduced.
    for (int sourceY = 0; sourceY < height; ++sourceY) {
        for (int sourceX = 0; sourceX < width; ++sourceX) {
            const uint8_t value = std::min<uint8_t>(15, src[(y + sourceY) * CONFIG_LCD_WIDTH + (x + sourceX)]) * 17;
            const int outputX = sourceX * scale;
            const int outputY = sourceY * scale;

            for (int dy = 0; dy < scale; ++dy) {
                uint8_t *row = pixelBuffer.get() + (outputY + dy) * outputWidth + outputX;
                std::fill(row, row + scale, value);
            }
        }
    }

    stbi_write_png(filename.c_str(), outputWidth, outputHeight, 1, pixelBuffer.get(), outputWidth);
}
