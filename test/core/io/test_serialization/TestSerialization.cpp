 ...
 * @brief Implements the TestSerialization component used by the unit test.
 *
...
void test_01_write_read() {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            reader.read(i32);
            reader.read(u32);
            TEST_ASSERT_TRUE((i8) == (int8_t(-i)));
            TEST_ASSERT_TRUE((u8) == (uint8_t(i)));
            TEST_ASSERT_TRUE((i16) == (int16_t(-i * 2)));
            TEST_ASSERT_TRUE((u16) == (uint16_t(i * 2)));
            TEST_ASSERT_TRUE((i32) == (int32_t(-i * 4)));
            TEST_ASSERT_TRUE((u32) == (uint32_t(i * 4)));
        }
