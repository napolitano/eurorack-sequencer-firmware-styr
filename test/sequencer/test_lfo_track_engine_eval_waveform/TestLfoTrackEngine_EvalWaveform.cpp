...
void test_01_basic_waveforms_at_phase_0_25() {
        SequencerHarness h;
        auto &app = h.app();
        ...
        TEST_ASSERT_TRUE(std::abs(v - float(l.hi())/100.f) <= 1e-2f);
        ...
        TEST_ASSERT_TRUE(std::abs(v - mid) <= 1e-2f);
        ...
        TEST_ASSERT_TRUE(std::abs(v - expected) <= 1e-2f);
        ...
        TEST_ASSERT_TRUE(v <= float(l.hi())/100.f + 1e-2f && v >= float(l.low())/100.f - 1e-2f);
...
int main() {
    UNITY_BEGIN();
    RUN_TEST(test_01_basic_waveforms_at_phase_0_25);
    return UNITY_END();
}
